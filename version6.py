...
for bar in range(0, 25):
    offset = 4 * bar # Get it back in sync
    # Add chord
    ...
    # Waltz
    n = c.notes[0]
    n.duration.quarterLength = 4 / 3
    harmony.insert(bar*4, n)
    c = chord.Chord(c.notes[1:])
    c.duration = duration.Duration(4/3)
    harmony.insert(bar*4 + 4/3, c)
    harmony.insert(bar*4 + 8/3, c.__deepcopy__())

    # Add notes
    while offset < (bar + 1) * 4:
